...
class Attack():
    ...
    def attack(self, imgs, txts, txt2img, device='cpu', max_length=30, scales=None, **kwargs):
        ...
        # N
        steps = 1
        adv_txts, momentum = self.multi_attacker.txt_attack(self.model, txts, imgs,txt2img, steps, momentum, scales=scales)
        steps = 10
        adv_imgs = imgs
        adv_imgs = self.multi_attacker.img_attack(self.model, adv_txts, adv_imgs,imgs,txt2img, steps, momentum,device, scales=scales)
        return adv_imgs, adv_txts
    
class CMIAttacker():
    def __init__(self, ref_net, tokenizer, cls=True, max_length=30,number_perturbation=1, topk=10, threshold_pred_score=0.3, batch_size=32, imgs_eps=2/255, step_size=0.5/255):
